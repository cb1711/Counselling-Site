...
class CounsellingChatFactory(WebSocketServerFactory):
	...
	query="INSERT INTO  IT_message(Text,Date,Time,Receiver_id,Sender_id,Email) VALUES('%s','%s','%s','%d','%d','%s')"%(payload,datetime.date.today(),datetime.datetime.now().time(),c["id"],c["recv_id"],str(self.cton[client]))
	cursor=db.cursor()
	cursor.execute(query)
	db.commit()
        if not c["partner"]:
